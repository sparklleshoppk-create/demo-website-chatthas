      ...
      {orders.map(order => (
        <div key={order.id} className={`bg-dark-card border rounded-sm flex flex-col h-full ${
          ...
          <div className={`p-4 border-b flex justify-between items-center ${
            ...
            <div>
              <p className="text-base font-black text-gold-500 uppercase tracking-widest">{order.order_number}</p>
              <div className="flex items-center gap-2 mt-1 text-sm text-cream/40 font-bold uppercase">
                <FaClock size={12} />
                <span>{new Date(order.created_at).toLocaleTimeString('en-US', { timeZone: 'Asia/Karachi', hour: '2-digit', minute: '2-digit' })}</span>
              </div>
            </div>
            <span className={`text-sm px-2 py-0.5 rounded uppercase font-black ${
              ...
              <div key={idx} className="flex justify-between items-start gap-4">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    {item.variant_label && (
                      <p className="text-sm text-gold-500/60 uppercase font-bold tracking-tighter">{item.variant_label}</p>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="p-4 border-t border-dark-border">
            <button 
              ...
              className={`w-full py-3 flex items-center justify-center gap-2 text-base font-black uppercase tracking-widest transition-all rounded-sm ${
            ...
            >
              {order.status === 'confirmed' && <><FaUtensils size={14}/> Start Cooking</>}
              {order.status === 'preparing' && <><FaCheck size={14}/> Mark Ready</>}
              {order.status === 'ready' && <><FaCheck size={14}/> Finish Order</>}
            </button>
          </div>
        </div>
      ))}
