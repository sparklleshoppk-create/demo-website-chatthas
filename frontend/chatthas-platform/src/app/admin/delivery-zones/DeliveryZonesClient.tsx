...
export default function DeliveryZonesClient({ initialZones, branches }: { initialZones: Zone[]; branches: Branch[] }) {
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <FaPlus size={14} /> Add Delivery Zone
      </button>

      {/* Add Zone Form */}
      {showForm && (
        <form onSubmit={handleCreate} className="bg-dark-card border border-dark-border p-8 rounded-sm space-y-6">
          <h3 className="text-sm font-display font-bold text-cream">New Delivery Zone</h3>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="space-y-2">
              <label className="text-base font-bold text-cream/40 uppercase tracking-widest">Zone Name</label>
              <input name="name" required placeholder="e.g. Near Zone" className="admin-input" />
            </div>

            <div className="space-y-2">
              <label className="text-base font-bold text-cream/40 uppercase tracking-widest">Branch</label>
            ...
            <div className="space-y-2">
              <label className="text-base font-bold text-cream/40 uppercase tracking-widest">Min Distance (km)</label>
              <input name="min_distance_km" type="number" step="0.1" defaultValue="0" required className="admin-input" />
            </div>

            <div className="space-y-2">
              <label className="text-base font-bold text-cream/40 uppercase tracking-widest">Max Distance (km)</label>
              <input name="max_distance_km" type="number" step="0.1" required placeholder="e.g. 5" className="admin-input" />
            </div>

            <div className="space-y-2">
              <label className="text-base font-bold text-cream/40 uppercase tracking-widest">Delivery Fee (Rs.)</label>
              <input name="delivery_fee" type="number" step="1" required placeholder="e.g. 150" className="admin-input" />
            </div>

            <div className="space-y-2">
              <label className="text-base font-bold text-cream/40 uppercase tracking-widest">Est. Time (min)</label>
      ...
      {Object.keys(grouped).length === 0 ? (
        <div className="bg-dark-card border border-dashed border-dark-border p-16 text-center rounded-sm">
          ...
          <p className="text-cream/20 text-base mt-1">Create zones to define distance-based delivery charges.</p>
        </div>
      ) : (
        Object.entries(grouped).map(([branchName, zones]) => (
          <div key={branchName} className="space-y-4">
            <h3 className="text-sm font-black text-gold-500 uppercase tracking-[0.2em] flex items-center gap-2">
              <FaMapMarkerAlt size={14} /> {branchName}
            </h3>
                ...
                <div key={zone.id} className={`bg-dark-card border rounded-sm p-6 space-y-4 transition-all ${zone.is_active ? 'border-dark-border' : 'border-dark-border opacity-50'}`}>
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-base text-cream/40 mt-1">{zone.min_distance_km} — {zone.max_distance_km} km</p>
                    </div>
                    <button onClick={() => handleToggle(zone)} className="text-cream/40 hover:text-gold-500 transition-colors">
                      {zone.is_active ? <FaToggleOn size={12} className="text-gold-500" /> : <FaToggleOff size={12} />}
                    </button>
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex items-center gap-2">
                      <FaTruck size={14} className="text-gold-500/60" />
                      <div>
                        <p className="text-sm text-cream/30 uppercase tracking-widest">Fee</p>
                        <p className="font-bold text-cream">Rs. {zone.delivery_fee}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <FaClock size={14} className="text-gold-500/60" />
                      <div>
                        <p className="text-sm text-cream/30 uppercase tracking-widest">Time</p>
                  ...
                  <button
                    ...
                    className="text-sm text-ember-500/60 hover:text-ember-500 uppercase tracking-widest font-bold transition-colors"
                  >
                    <FaTrash size={12} className="inline mr-1" /> Remove
                  </button>
                </div>
              ))}
            </div>
