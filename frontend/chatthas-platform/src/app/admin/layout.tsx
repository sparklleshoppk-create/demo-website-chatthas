...
export default function AdminLayout({
  ...
  return (
    <div className="flex h-screen bg-charcoal text-cream font-body overflow-hidden" style={{ zoom: 0.9 }}>
      <OrderNotification />
      {/* Sidebar */}
            ...
            {navigation.map((item) => {
              ...
              return (
                <li key={item.name}>
                  ...
                  >
                    <item.icon className={`mr-3 h-4 w-4 ${isActive ? 'text-gold-400' : 'text-cream/50'}`} />
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-16 bg-charcoal border-b border-dark-border flex items-center justify-between px-6 flex-shrink-0">
          <h1 className="text-xl font-display font-light italic gold-text">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-body font-medium text-cream">Admin User</span>
              {/* Note: In a real server component layout, we'd fetch the role from DB. 
                  Since this is a client component, we'll display a generic badge 
                  until we build out the full profile context API. */}
              <span className="text-[10px] font-bold tracking-widest text-gold-500 uppercase">Super Admin</span>
            </div>
          </div>
        </header>

        <main className="flex-1 overflow-y-auto p-8 bg-primary-black">
