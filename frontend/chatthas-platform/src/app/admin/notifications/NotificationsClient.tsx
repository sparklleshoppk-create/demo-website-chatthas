...
export default function NotificationsClient({ initialTemplates, recentLogs }: { initialTemplates: any[]; recentLogs: any[] }) {
  ...
  return (
    <div className="space-y-8">
          ...
          <div key={template.id} className="bg-dark-card border border-dark-border rounded-sm overflow-hidden">
            ...
            <div className="p-6 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <h3 className="font-display font-bold text-cream text-sm">
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-black uppercase tracking-widest border transition-all ${
                ...
                <button
                  ...
                  className="text-base text-cream/40 hover:text-gold-500 transition-colors uppercase font-bold tracking-widest"
                >
              ...
              <form onSubmit={(e) => handleSaveTemplate(e, template.id)} className="border-t border-dark-border p-6 space-y-6 bg-charcoal/50">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-blue-400 uppercase tracking-widest flex items-center gap-1"><FaSms size={12} /> SMS Template</label>
                    <textarea
                      name="sms_template"
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-green-400 uppercase tracking-widest flex items-center gap-1"><FaWhatsapp size={12} /> WhatsApp Template</label>
                    <textarea
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-bold text-gold-400 uppercase tracking-widest flex items-center gap-1"><FaEnvelope size={12} /> Email Subject</label>
                    <textarea
                      name="email_template"
                ...
                <div className="flex items-center gap-2 text-sm text-cream/30">
      ...
      <div className="space-y-4">
        <h3 className="text-sm font-display font-bold text-cream flex items-center gap-2">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-dark-border bg-charcoal">
                  <th className="px-6 py-3 text-sm font-bold text-cream/40 uppercase tracking-widest">Channel</th>
                  <th className="px-6 py-3 text-sm font-bold text-cream/40 uppercase tracking-widest">Event</th>
                  <th className="px-6 py-3 text-sm font-bold text-cream/40 uppercase tracking-widest">Recipient</th>
                  <th className="px-6 py-3 text-sm font-bold text-cream/40 uppercase tracking-widest">Status</th>
                  <th className="px-6 py-3 text-sm font-bold text-cream/40 uppercase tracking-widest">Sent</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-dark-border">
                {recentLogs.map(log => (
                  <tr key={log.id} className="hover:bg-charcoal/50">
                    ...
                    <td className="px-6 py-3">
                      <span className={`text-sm px-2 py-0.5 rounded-full border font-bold uppercase ${
                        log.status === 'sent' ? 'border-green-500/30 text-green-400' : 'border-ember-500/30 text-ember-400'
                      }`}>
                        {log.status}
                      </span>
                    </td>
                    <td className="px-6 py-3 text-base text-cream/40">{new Date(log.created_at).toLocaleString('en-US', { timeZone: 'Asia/Karachi', dateStyle: 'medium', timeStyle: 'short' })}</td>
                  </tr>
                ))}
              </tbody>
            </table>
