...
export default function MenuItemForm({ 
  ...
  // Phase 5: Dietary Flags
  const [dietaryFlags, setDietaryFlags] = useState<string[]>(item?.dietary_flags || []);

  const toggleFlag = (flag: string) => {
    if (dietaryFlags.includes(flag)) {
      setDietaryFlags(dietaryFlags.filter(f => f !== flag));
    } else {
      setDietaryFlags([...dietaryFlags, flag]);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.set('dietary_flags', JSON.stringify(dietaryFlags));
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2">
          ...
          <div className="flex items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-base text-cream/40 mt-2">JPG, PNG, or WebP. Max 5MB.</p>
              {uploadProgress && (
                <p className="text-base text-gold-400 mt-1 animate-pulse">{uploadProgress}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-2 border-t border-dark-border pt-6 mt-2">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-display font-bold text-cream">Variants (e.g. Sizes)</h3>
            ...
            <div key={vIdx} className="bg-dark-card border border-dark-border p-4 rounded-sm mb-4">
              ...
              <div className="space-y-2 pl-4 border-l-2 border-dark-border">
                ...
                <button type="button" onClick={() => {
                  ...
                  setVariants(newV);
                }} className="text-base font-bold text-gold-500 hover:text-gold-400 mt-2">
                  + Add Option
                </button>
              </div>
            </div>
        ...
        <div className="md:col-span-2 border-t border-dark-border pt-6 mt-2">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-display font-bold text-cream">Add-ons (e.g. Extras)</h3>
        ...
        <div className="md:col-span-2 border-t border-dark-border pt-6 mt-2">
          <h3 className="text-sm font-display font-bold text-cream mb-4">Dietary Flags</h3>
          <div className="flex flex-wrap gap-4">
            {['vegetarian', 'vegan', 'halal', 'gluten_free', 'contains_nuts', 'spicy'].map((flag) => (
              <label key={flag} className="flex items-center gap-2 cursor-pointer bg-charcoal border border-dark-border px-4 py-2 rounded-sm hover:border-gold-500/50 transition-colors">
                <input
                  type="checkbox"
                ...
                />
                <span className="text-base font-bold uppercase tracking-widest text-cream/80">{flag.replace('_', ' ')}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="md:col-span-2 flex items-center gap-8 border-t border-dark-border pt-6">
          ...
          <div className="flex items-center gap-3">
             <label className="text-base text-cream/40 uppercase font-bold tracking-widest">Initial Stock</label>
             <input 
               type="number" 
               name="stock_quantity"
