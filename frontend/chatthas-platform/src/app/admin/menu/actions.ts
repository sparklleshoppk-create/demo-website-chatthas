...
const menuItemSchema = z.object({
  ...
  discount_price: z.coerce.number().min(0, 'Discount price must be positive').nullable().optional(),
  discount_end_date: z.string().nullable().optional(),
  category_id: z.string().uuid('Select a valid category'),
  badge: z.enum(['bestseller', 'new', 'spicy', 'popular', '']).nullable().optional(),
...
export async function createMenuItem(formData: FormData) {
  ...
  const raw = {
    ...
    discount_price: formData.get('discount_price') ? formData.get('discount_price') : null,
    discount_end_date: formData.get('discount_end_date') || null,
    ...
    dietary_flags: formData.get('dietary_flags') ? JSON.parse(formData.get('dietary_flags') as string) : [],
...
// Seed function remains as is or can be updated if needed
export async function seedDatabase() {
    // We already have genSql.js for seeding which is more reliable.
    // This server action can be used to trigger the same logic if desired.
    return { error: "Please use the genSql.js script or Supabase SQL editor for seeding." };
}
